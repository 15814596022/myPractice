...
这项测试针对的是Feeds。
##### 1.1 are defined
数组allFeeds是存在的，并且不为空。
##### 1.2 has url
这些Feeds对象拥有各自的url属性。
##### 1.3 has name
这些Feeds对象拥有各自的name属性。
...
#### 3. Initial Entries
这项测试检测loadFeed 函数是否被正常调用
##### 3.1 loadFeed is work
检测到feed容器中至少存在一个.entry的元素

---

#### 4. New Feed Selection
创建一个新的feed以检测 loadFeed 函数加载一个新源的时候内容会真的改变。
##### 4.1 loadFeed could change
导入新的feed后内容会发生改变